    ...
    * */
    ...
    FlatFileItemReader<TransacaoCNAB> reader() {
        return new FlatFileItemReaderBuilder<TransacaoCNAB>()
                .name("reader")
                .resource(new FileSystemResource("CNAB.txt"))
                .fixedLength()
    ...
    ItemProcessor<TransacaoCNAB, Transacao> processor() {
        return item -> {
            ...
            ).withValor(item.valor().divide(BigDecimal.valueOf(100), RoundingMode.HALF_DOWN))
                    .withData(item.data())
                    .withHora(item.hora());
            return transacao;
        };
    ...
    JdbcBatchItemWriter<Transacao> writer(DataSource dataSource) {
        return new JdbcBatchItemWriterBuilder<Transacao>()
                ...
                .sql("INSERT INTO transacoes (tipo, data, valor, cpf, cartao, hora, dono_da_loja, nome_da_loja) " +
